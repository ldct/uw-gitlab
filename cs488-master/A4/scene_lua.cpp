...
int gr_sphere_cmd(lua_State* L)
{
  ...
  const char* name = luaL_checkstring(L, 1);
  data->node = new GeometryNode( name, new NonhierSphere(glm::vec3(0.0), 1.0) );
...
int gr_cube_cmd(lua_State* L)
{
  ...
  const char* name = luaL_checkstring(L, 1);
  data->node = new GeometryNode(name, new NonhierBox(glm::vec3(0.0), 1.0));
...
extern "C"
int gr_procedural_material_cmd(lua_State* L)
{
  GRLUA_DEBUG_CALL;
  ...
  double a_kd[3], a_ks[3];
  get_tuple(L, 1, a_kd, 3);
  get_tuple(L, 2, a_ks, 3);
  double a_shininess = luaL_checknumber(L, 3);

  double b_kd[3], b_ks[3];
  get_tuple(L, 4, b_kd, 3);
  get_tuple(L, 5, b_ks, 3);
  double b_shininess = luaL_checknumber(L, 6);

  double bump_factor = luaL_checknumber(L, 7);
  double blend_octaves = luaL_checknumber(L, 8);

  data->material = new ProceduralPhong(
    glm::vec3(a_kd[0], a_kd[1], a_kd[2]),
    glm::vec3(a_ks[0], a_ks[1], a_ks[2]),
    a_shininess,
    glm::vec3(b_kd[0], b_kd[1], b_kd[2]),
    glm::vec3(b_ks[0], b_ks[1], b_ks[2]),
    b_shininess,
    bump_factor, blend_octaves
  );
...
static const luaL_Reg grlib_functions[] = {
  ...
  {"procedural_material", gr_procedural_material_cmd},
