...
{
	if (x < 0 || x >= m_dim || y < 0 || y >= m_dim) return 0;
	return m_values[ y * m_dim + x ];
}

void Maze::setValue( int x, int y, int h )
{
	if (x < 0 || x >= m_dim || y < 0 || y >= m_dim) return;
...
vector<tuple<int,int>> Maze::blocks() {
	int i,j;
	auto ret = vector<tuple<int,int>>();
	for (i=0; i<m_dim; i++) {
		for (j=0; j<m_dim; j++) {
			if ( getValue(i,j)==1 ) {
				ret.push_back(make_tuple(i,j));
			}
		}
	}
	return ret;
