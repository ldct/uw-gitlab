...
int gr_joint_cmd(lua_State* L)
{
  ...
  for (int i=2; i<=3; i++) {
    ...
    luaL_argcheck(L, luaL_len(L, 2) == 3, i, "Three-tuple expected");
  }
...
int gr_mesh_cmd(lua_State* L)
{
  ...
  if (lua_gettop(L) == 4) {
    float aux = luaL_checknumber(L, 3);
    static_cast<GeometryNode*>(data->node)->m_aux = aux;
  }
...
// Add a child to a node
extern "C"
int gr_node_set_selection_cmd(lua_State* L)
{
  GRLUA_DEBUG_CALL;
...
static const luaL_Reg grlib_node_methods[] = {
  ...
  {"set_selection", gr_node_set_selection_cmd},
